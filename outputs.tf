...
output "iot_security_solutions_recommendations_enabled" {
  ...
  value       = { for k, v in azurerm_iot_security_solution.iot_security_solutions : k => one(v.recommendations_enabled) if v.recommendations_enabled != null && length(v.recommendations_enabled) > 0 }
}
